...
-- 5
with cte as (
select c.customer_id,c.first_name,c.last_name,c.country,sum(i.total) as total_reven, rank() over(partition by c.country order by sum(i.total) desc) as rnk
...
-- 6
with cte_two as (
select c.customer_id,c.first_name,c.last_name,t.name,sum(il.quantity) as total_quantity, dense_rank() over(partition by c.customer_id order by sum(il.quantity) desc) as rnkd
from customer c join invoice i on c.customer_id = i.customer_id
join invoice_line il on i.invoice_id = il.invoice_id
join track t on il.track_id = t.track_id
...
select 
(select count(*) from customer) as total_customer_count ,
(select count(*) from customer where customer_id not in(select customer_id from active_customer)) as churned_customers,
round((select count(*) from customer where customer_id not in(select customer_id from active_customer)) / (select count(*) from customer) *100,2) as churn_percentage;
...
-- 12
select max(date(invoice_date)) as now_date from invoice;
with purchase_customers as(
select distinct customer_id from invoice 
where invoice_date>= date_sub('2020-12-30',interval 90 day)
)

select c.customer_id, concat(c.first_name,' ',c.last_name) as cust_names
from customer c
where c.customer_id not in (select customer_id from purchase_customers)
limit 5;
